...
export function AdminPanel({ session, onSignOut }: AdminPanelProps) {
      ...
      <div className="flex min-h-screen items-center justify-center bg-[#0f1113] px-6 text-center text-white">
        <div className="max-w-md">
          <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full border-2 border-[#b90046]/30">
            <Settings size={28} className="text-[#b90046]" />
          </div>
          <p className="font-serif text-3xl">Access denied</p>
          <p className="mt-4 text-sm leading-6 text-white/50">
            Your account does not have admin access. Please contact the firm administrator to be granted access.
          </p>
          <button
            onClick={handleSignOut}
            className="mt-8 rounded-lg border border-[#df8eae] px-6 py-3 text-xs font-bold uppercase tracking-[0.14em] text-[#f5c4d5] transition-colors hover:bg-[#df8eae] hover:text-[#0f1113]"
          >
            Sign out
          </button>
        </div>
  ...
  const navItems: { key: AdminTab; label: string; icon: typeof Inbox; group: string }[] = [
    ...
    { key: 'content', label: 'Site Content', icon: Settings, group: 'Content' },
  ...
  return (
    <div className="flex min-h-screen bg-[#f7f8f8]">
      ...
      <aside className="fixed inset-y-0 left-0 z-30 flex w-64 flex-col border-r border-[#e1e5e5] bg-white">
              ...
              {navItems.filter((n) => n.group === group).map((item) => (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all ${
                    tab === item.key
                      ? 'bg-[#fdf0f5] text-[#b90046] shadow-sm'
                      : 'text-[#4d585d] hover:bg-[#f7f8f8] hover:text-[#b90046]'
                  }`}
                >
                  <item.icon size={17} />
                  {item.label}
        ...
        <div className="border-t border-[#e1e5e5] px-5 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#b90046] text-sm font-bold text-white">
            ...
            <button
              onClick={handleSignOut}
              className="rounded-md p-2 text-[#687277] transition-colors hover:bg-[#f7f8f8] hover:text-[#b90046]"
              aria-label="Sign out"
            >
              <LogOut size={16} />
            </button>
      ...
      <div className="ml-64 flex-1">
        {/* Top bar */}
        <div className="sticky top-0 z-20 flex items-center justify-between border-b border-[#e1e5e5] bg-white/90 px-8 py-4 backdrop-blur">
          <h1 className="font-sans text-lg font-bold text-[#273237]">
            {navItems.find((n) => n.key === tab)?.label}
          </h1>
          <button
            onClick={() => navigateTo('/')}
            className="flex items-center gap-2 rounded-lg border border-[#e1e5e5] px-4 py-2 text-xs font-bold uppercase tracking-[0.12em] text-[#687277] transition-colors hover:border-[#b90046] hover:text-[#b90046]"
          >
        ...
        <div className="p-8">
          ...
          {tab === 'settings' && (
            <div className="rounded-xl border border-[#e1e5e5] bg-white p-8 text-sm text-[#687277]">
              <p>Account and settings management will appear here.</p>
            </div>
          )}
        </div>
      </div>
